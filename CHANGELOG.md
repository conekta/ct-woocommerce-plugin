...
- Fix: the order total now matches the WooCommerce total to the cent on every path. `unit_price` is `line_subtotal / quantity` rounded to cents, so `unit_price × quantity` (plus tax rounding) could drift a cent or two; `build_snapshot` (card) now reconciles via `ckpg_check_balance`, and `ckpg_check_balance` itself was fixed to absorb the rounding delta in BOTH directions (it previously only ever added `abs()`, over-charging when the lines already exceeded the total — affecting cash, bank transfer, BNPL and pay by bank).
