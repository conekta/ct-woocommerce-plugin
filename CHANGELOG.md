## [6.0.6]() - 2026-06-30
- Fix: tax-inclusive stores no longer report IVA as a `dynamic_pricing` discount on the Conekta order. The price-level (dynamic pricing) detection compared a gross `get_regular_price()` against the net `line_subtotal`, so the tax difference was misread as a discount (e.g. a $2,610 tax-inclusive item showed a phantom -$360 "Descuento"). The regular price is now normalized to net via `wc_get_price_excluding_tax()` before the comparison when the store enters prices tax-inclusive; tax-exclusive stores are unaffected. Applies to both order-build paths: `build_snapshot` (card, classic + Blocks) and `ckpg_build_line_items` (cash, bank transfer, BNPL, pay by bank).
- Feature: each Conekta line item now carries a `tax_included` metadata flag indicating whether its `unit_price` already includes tax. True only when the store is tax-inclusive AND the product is taxable (tax-exempt products report `false`). Added to all payment methods.
- Tests: regression coverage for tax-inclusive vs tax-exclusive pricing (no phantom discount, real discounts still detected) and for the `tax_included` flag across taxable/exempt products. 173 tests pass.
