...
- Fix: the card order total now matches the WooCommerce cart total to the cent. `unit_price` is `line_subtotal / quantity` rounded to cents, so `unit_price × quantity` (plus tax rounding) could drift a cent or two; `build_snapshot` now reconciles the rounding into the tax line so Conekta charges exactly the WC total (the card path doesn't run `ckpg_check_balance`).
