...
- Feature: reconciler job (`conekta-reconciler.php`, Action Scheduler every 15 min). Polls Conekta for `pending`/`failed`/`checkout-draft` orders with `conekta-order-id` meta modified in the last 48h: paid → complete (with duplicate + amount guards), expired/canceled → cancel. Heals stores whose webhook is broken or never configured — no external dependency.
