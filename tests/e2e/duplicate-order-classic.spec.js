...
h.run('Classic Checkout — duplicate-order guard', { checkoutType: 'classic', product: h.STAGING_VARIABLE_PRODUCT }, async ({ page, assert, config, STORE_URL, BILLING, TEST_CARD }) => {
  ...
  const isCheckoutRequest = url => url.href.includes('conekta_checkout_request');
  const blockCheckoutRequest = route => route.abort();
  await page.route(isCheckoutRequest, blockCheckoutRequest);
  ...
  await page.unroute(isCheckoutRequest, blockCheckoutRequest);
  ...
  const paidId = paid[0] ? paid[0].id : 0;
  const duplicates = await h.findDuplicateOrders({ email: BILLING.email, afterOrderId: paidId });
  const dupIds = duplicates.map(o => `#${o.id}(${o.status})`).join(', ');
  assert(duplicates.length === 1, `exactly ONE flagged duplicate order was created after #${paidId} (got ${duplicates.length}: ${dupIds || 'none'})`);
